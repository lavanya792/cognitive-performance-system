...
```
┌─────────────────────────────────────────────────────┐
│  CPS COGNITIVE  |  Reaction  Stroop  Fatigue  Score  │
├─────────────────────────────────────────────────────┤
│                                                     │
│         ┌─────────────────────────┐                 │
│         │   REACTION TIME TEST    │                 │
...
│         └─────────────────────────┘                 │
│                                                     │
│   Avg: 251ms    Best: 218ms    Std Dev: 28ms        │
└─────────────────────────────────────────────────────┘
```
...
```r
install.packages(c("shiny", "ggplot2", "later", "scales"))
```

- R >= 4.1
- No database or external API needed — runs fully locally
...
shiny::runApp("path/to/cps-app")
```

Or directly in RStudio — open `app.R` and click **Run App**.
...
shinylive::export(appdir = "cps-app", destdir = "cps-app/docs")

# 3. Push to GitHub
# Then: repo Settings → Pages → Branch: main, Folder: /docs → Save
```

Your app goes live at:
```
https://YOUR_USERNAME.github.io/cps-app/
```
...
├── docs/          ← Auto-generated by shinylive (don't edit)
│   ├── index.html
│   └── shinylive/
└── README.md
```
...
```
Composite Score = (100 / avg_reaction_time_sec)
                + (stroop_accuracy × 100)
                + consistency_bonus (20pts if SD < 200ms)
```

Higher is better. A perfect performer with 200ms avg reaction time and 100% Stroop accuracy scores around **620**.
